...
pub async fn machine_index_header_middleware(request: Request, next: Next) -> Response {
    ...
    if path.starts_with("/api") {
        ...
        if machine_index.is_none() {
            return Json(failed::<()>("请在header传Machineindex".to_string())).into_response();
        }
        ...
        let machine_index_str = match machine_index_str {
            Ok(v) => v,
            Err(_) => return Json(failed::<()>("Machineindex必须为字符串".to_string())).into_response(),
        };

        // parse to int
        let machine_index = match machine_index_str.parse::<u32>() {
            Ok(v) => v,
            Err(_) => return Json(failed::<()>("Machineindex必须为数字".to_string())).into_response(),
        };

        if machine_index == 0 {
            return Json(failed::<()>("Machineindex不能为0".to_string())).into_response();
        }
    }
